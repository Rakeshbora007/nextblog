      ...
      <div className="flex flex-col sm:flex-row gap-5 w-full">
        <Link href={`blog/${filteredFeaturedPosts[0]?._id}`} className="relative w-full sm:w-[50%] md:w-[60%] lg:w-[70%] xl:w-[85%] h-[260px] sm:h-[418px] md:h-[520px] lg:h-[620px] xl:h-[730px]">
          <Image alt="s" className="object-cover rounded-lg" src={filteredFeaturedPosts[0]?.image} fill priority
            sizes="(min-width: 1920px) 1070px, (min-width: 1280px) calc(52.74vw + 68px), (min-width: 1040px) calc(58.64vw - 62px), (min-width: 920px) calc(55vw - 59px), (min-width: 780px) calc(30.83vw + 159px), (min-width: 640px) calc(50vw - 24px), calc(100vw - 28px)" />
          ...
          {[1, 2].map((e) => (
            <>
              <Link href={`blog/${filteredFeaturedPosts[e]?._id}`} className="relative w-full h-[100px] sm:h-[200px] md:h-[250px] lg:h-[300px] xl:h-[354px] max-sm:h-[180px]">
                <Image alt="s" className="rounded-lg object-cover" src={filteredFeaturedPosts[e]?.image} fill priority sizes="(min-width: 1920px) 630px, (min-width: 1280px) calc(31.13vw + 39px), (min-width: 1040px) calc(41.36vw - 38px), (min-width: 920px) calc(45vw - 41px), (min-width: 780px) calc(25.83vw + 131px), calc(50vw - 24px)" />
                <div className="text-white max-sm:pb-3 max-md:pl-2 pb-10 pl-10 flex-col absolute top-0 left-0 w-full h-full flex gap-6  max-md:gap-2 items-start justify-end">
                  ...
                  <h1 className="w-[100%] xl:text-[40px] font-bold xl:leading-[48px]">{filteredFeaturedPosts[e]?.title}</h1>
                </div>
              </Link>
            </>
          ))}
